...
# -------------------
# Semantic Index (single embedding column)
# -------------------
class SemanticIndex:
    def __init__(self):
    ...
    def build(self, df: pd.DataFrame, batch_size: int = 5000):
        ...
        # use a single column "embedding_text"
        docs, metadatas, ids = [], [], []
        for i, row in df.iterrows():
            # combine only human-meaningful info
...
# Filtering logic
# -------------------
def apply_filters(df: pd.DataFrame, search_text: str):
    if not search_text.strip():
        return df
    idxs = st.session_state.semantic_idx.query(search_text, n_results=1000)
    if len(idxs) == 0:
        return df
    return df.loc[sorted(list(idxs))].copy()
...
def show_dashboard(dtos):
    ...
    if st.button("Run NLP Search"):
        with st.spinner("🔎 Searching..."):
            ...
            if len(idxs) > 0:
                filtered = df.loc[sorted(list(idxs))].copy()
            else:
        ...
        if filtered.empty:
            ...
            # Stats
            st.subheader("📊 Min / Max / Avg Amount per Item per City")
