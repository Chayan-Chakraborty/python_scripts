...
def fetch_excel_data(db_config , curr_date, start_date, environment_name):
    ...
    unique_Users_creating_dp_query =  f"SELECT COUNT(DISTINCT created_by_user_id) AS Count FROM cms.digital_profiles dp left join shield.`app_user_role_mapping` ul on dp.`contractor_uuid` = ul.user_id WHERE dp.created_at between '{curr_date} 00:00:00' AND '{curr_date} 23:59:59' and ul.role_id = 1 and dp.`contractor_uuid` NOT IN{internal_users};"
    ...
    total_unique_Users_creating_dp_query = f"SELECT COUNT(DISTINCT created_by_user_id) AS Count FROM cms.digital_profiles dp left join shield.`app_user_role_mapping` ul on dp.`contractor_uuid` = ul.user_id WHERE dp.created_at <= '{curr_date} 23:59:59' and ul.role_id = 1 and dp.`contractor_uuid`  NOT IN{internal_users};"
    # End ----------------------------------------------
...
#main('prod',None, None)
main('prod', '2024-09-26' , '2024-10-08')
