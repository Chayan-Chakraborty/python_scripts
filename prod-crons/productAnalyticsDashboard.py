...
def backfill_data(start , end, db_config, destination_table, columns, environment_name):
    res_date = start
    while res_date <= end:
        insert_data_into_table(fetch_excel_data(db_config, res_date, environment_name), destination_table, columns, db_config)
        res_date += timedelta(days=1)
    
def fetch_excel_data(db_config , curr_date, environment_name):
    module_path = '/home/ec2-user/common/FetchInternalUser.py'
    ...
    Quotations_Shared_query = f"Select Count(*) as Count from vishanti.estimator e left join shield.`app_user_role_mapping` ul on e.user_id = ul.user_id where ul.role_id = 1 and  e.status IN ('FINALIZED') and e.`updated_at` between '{curr_date} 00:00:00'  and '{curr_date} 23:59:59' and e.`user_id`  NOT IN{internal_users}"
...
def main(environment_name, startDate, endDate):
    try : 
        ...
        if endDate:
            ...
            insert_data_into_table(fetch_excel_data(db_config, (datetime.now() - timedelta(1)).strftime('%Y-%m-%d'), environment_name), destination_table, columns, db_config)
...
main('prod',None, None)
#main('prod', '2024-08-08' , '2024-08-12')
