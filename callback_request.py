...
email_configs = {
    'local': {
        ...
        'subject': f'CALLBACK-REQUESTS-Data({(datetime.now()).strftime("%Y-%m-%d")})',
    ...
    'prod': {
        ...
        'password': 'gpbs zfep pdwk awgv',
        'from_email': 'app_store_dev@pidilite.vc',
        'to_emails': ['chayan.chakraborty@pidilite.vc, ahona@pidilite.vc'],  # List of recipients
        'subject': f'CALLBACK-REQUESTS-Data({(datetime.now()).strftime("%Y-%m-%d")})',
        'body': 'Please find the attached Excel file with the data.'
    }
}
...
def main(environment_name):
    query = f"Select cr.id, cr.external_id, cr.status, cr.phone_number, cr.module, cr.channel, cr.name, cr.city, cr.state, cr.metadata, cr.created_at, cr.updated_at from callback_requests cr where cr.module='DAAS' AND cr.created_at between DATE_SUB(CURDATE(), INTERVAL 7 DAY) and CURDATE()"
    all_data = []
    ...
    filename = 'CALLBACK-REQUESTS-Data.xlsx'
